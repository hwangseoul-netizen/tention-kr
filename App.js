// App.js — TENtion KR v1.4.6 (Snack-ready, single file)
// - 4카테고리 한 줄 고정 그리드
// - 거리/시간 스텝퍼 확대 + 우측 "정렬 기준" 버튼(작게)
// - 지역선택 칩 여백/테두리 정리
// - 카드 컴팩트화(여러 슬롯 동시 노출 ↑)
// - Details 풀스크린 오버레이 + 가독성 개선
// - 생성모달 카테고리 칩 텍스트 중앙정렬
// - 외부 라이브러리 의존성 없음(Expo Snack OK)
...
const T = {
  ...
  cat:{Dating:"데이트 1:1", Friends:"친구 1:1", Workout:"운동 1:1", Talk:"토크 1:1"},
  host:{me:"내가 주최", plat:"TENtion 주최"},
  createTitle:"10분 슬롯 만들기",
  category:"카테고리",
  hostLabel:"주최",
  pickCityTime:"도시 & 시간",
  startEnd:"시작/종료 (24H)",
...
const CATS = [
  ...
  {key:"Friends", label:"친구 ",    icon:"🤝", color:"#2EE778"},
  {key:"Workout", label:"운동 ",    icon:"💪", color:"#FFA23B"},
...
/* ---- countdown helpers ---- */
function buildTodayTs(hm){ const now=new Date(); const [h,m]=hm.split(":").map(Number); return new Date(now.getFullYear(),now.getMonth(),now.getDate(),h,m,0).getTime(); }
...
function fmtHMS(secs){ const h=Math.floor(secs/3600); const m=Math.floor((secs%3600)/60); const s=secs%60; return `${pad2(h)}:${pad2(m)}:${pad2(s)}`; }
...
const KR_PLACES = {
  ...
  GN:["삼성역 코엑스몰","선릉 공원","역삼 로비"], HD:["홍대입구 9번","합정 메세나폴리스","망원 한강"], JS:["석촌호수","잠실새내","잠실운동장 앞"], GS:["성수연방","서울숲 벤치","건대입구 로터리"], YD:["IFC몰 로비","63빌딩 앞","여의서로 산책로"],
  ICN:["송도 센트럴파크","인천대공원"], GGN:["일산 호수공원","의정부역 광장"], GGS:["분당 정자역","판교 알파돔"], SUW:["수원역 AK앞","광교 호수공원"],
  BUS:["광안리 해변","해운대 블루라인"], DG:["동성로 거리","수성못"], DJ:["둔산 타임월드"], GJ:["충장로"], USN:["태화강 국가정원"]
};

const TALK_TOPICS = ["멘탈/회복","자기계발","책 토크","창업/비즈","마케팅","성장통","커리어","대인관계","여행","스포츠","OTT/영화","MBTI","연애상담"];
...
function generateKRSlots({ cityCode="GN", band="저녁", count=20 }){
  ...
  for(let i=0;i<count;i++){
    ...
    const topic = (type==="Talk") ? TALK_TOPICS[Math.floor(Math.random()*TALK_TOPICS.length)] : null;
    ...
    list.push({
      ...
      title: `${place} • ${type==="Talk" ? (topic||"Talk") : cityName(cityCode)}`,
...
function Root(){
  ...
  // 지역 선택(다중): 기본 HOT5
  const [selectedCities,setSelectedCities]=useState(new Set(HOT5));
  const toggleCity=(code)=>{
    setSelectedCities(prev=>{ const s=new Set(prev); s.has(code)?s.delete(code):s.add(code); return s; });
  };
  ...
  useEffect(()=>{
    ...
    const base   = cities.length? cities : HOT5;
  ...
  const resetHome=()=>{
    setActiveCat(""); setRadius(5); setDur(10);
    setSortBy(t.sortOpt[0]); setSearch(""); setMyOnly(false);
  ...
  const list = useMemo(()=>{
    ...
    if(selectedCities.size) arr = arr.filter(s=> selectedCities.has(s.city));
    arr = arr.filter(s => s.totalMins>=dur);
    if(myOnly) arr = arr.filter(s => s.attendees.includes(ME) || s.hostType==="me");
    if(search.trim()){
    ...
    if(sortBy===t.sortOpt[0]) arr.sort((a,b)=>(a.minsLeft||1e9)-(b.minsLeft||1e9)); // 마감임박
    if(sortBy===t.sortOpt[1]) arr.sort((a,b)=>b.id-a.id);                            // 최신
    if(sortBy===t.sortOpt[3]) arr.sort((a,b)=>(b.proofScore||0)-(a.proofScore||0));  // 평점
    return arr;
  ...
  const join=(slot)=>{
    ...
    if(slot.attendees.length>=slot.max){ Alert.alert("Full","This slot is already full."); return; }
    const next={...slot, attendees:[...slot.attendees,ME]};
    setSlots(prev=>prev.map(s=>s.id===slot.id?next:s));
    Alert.alert("체크인 완료"," ");
  };
  const leave=(slot)=>{
    if(!slot.attendees.includes(ME)) return;
    setSlots(prev=>prev.map(s=>s.id===slot.id?{...s,attendees:s.attendees.filter(x=>x!==ME)}:s));
  };
  const shareSlot=async(slot)=>{
    try{ await Share.share({message:`TENtion • ${slot.title}\n${slot.type} @ ${cityName(slot.city)} • ${slot.band}`}); }catch(e){}
  };

  // Create form
  const [createOpen,setCreateOpen]=useState(false);
  const [form,setForm]=useState({ cat:"", host:T.host.me, city:"GN", start:"18:00", end:"18:10", dur:10, title:"", desc:"" });

  const openCreate=()=>{
    const firstSel = Array.from(selectedCities)[0] || "GN";
    setForm(f=>({ ...f, cat: activeCat || "", city: firstSel, dur:10, start:"18:00", end:"18:10", _cityPick:false }));
    setCreateOpen(true);
  };
  ...
  const createSlot=()=>{
    ...
    const s = {
      ...
      title: form.title || defaultTitle(form.cat || "Talk"),
      start: form.start,
      end,
      totalMins: mins,
      minsLeft: Math.max(5, Math.floor(mins*0.8)),
      attendees: [], max:2, desc: form.desc || defaultDesc(form.cat || "Talk"),
      proofScore:0,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
          ...
          {CATS.map(c=>{
            ...
            return (
              <TouchableOpacity key={c.key}
                ...
                style={[
                  styles.catChip, {borderColor:c.color},
        ...
        {/* Row: distance • duration • sort (정렬 기준) */}
        <View style={styles.row3}>
          <Stepper label={t.distance} value={radius}
                   onMinus={()=>setRadius(clamp(radius-KM_STEP,1,50))}
                   onPlus={()=>setRadius(clamp(radius+KM_STEP,1,50))} />
          <Stepper label={t.duration} value={dur} step={10}
        ...
        <View style={styles.hotRow}>
          {HOT5.map(code=>(
            <TouchableOpacity key={code}
              ...
              style={[styles.cityChip, selectedCities.has(code) && styles.cityChipActive]}>
              <Text style={[styles.cityChipT, selectedCities.has(code) && styles.cityChipTActive]} numberOfLines={1}>
                {cityName(code)}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.noteBox}><Text style={styles.note}>{t.safetyNote}</Text></View>
      </ScrollView>

      {/* SHEETS */}
      {showSortSheet && <ActionSheet title={t.sort} value={sortBy} options={t.sortOpt}
        onPick={(v)=>{ setSortBy(v); setShowSortSheet(false); }} onCancel={()=>setShowSortSheet(false)} />}

      {/* 다중 지역 선택 시트 */}
      {showCitySheet && (
        <MultiCitySheet
          currentSet={selectedCities}
          onApply={(codes)=>{ setSelectedCities(new Set(codes)); setShowCitySheet(false); }}
...
function Card({slot,onDetails,onPrimary,nowMs}){
  if(!slot) return null;
  const {state, secsToStart} = getState(nowMs, slot.start, slot.totalMins);
  const tint  = tintByMins(slot.minsLeft);
  const ratio = slot.totalMins ? Math.max(0, slot.minsLeft)/slot.totalMins : 0;
  const isFull= slot.attendees.length>=slot.max;
    ...
    <View style={[styles.card,{borderColor:colorFor(slot.type)}]}>
      <View style={styles.cardHead}>
        <Text style={[styles.cardType,{color:colorFor(slot.type)}]} numberOfLines={1}>
          {iconFor(slot.type)} {slot.type}  •  {cityName(slot.city)}  •  {slot.band}
        </Text>
        <View style={{flexDirection:"row",gap:6,alignItems:"center"}}>
          {rightBadge}
          {isFull && <MiniBadge text="Full" tone="#FF6B6B"/>}
        </View>
      </View>

      <Text style={styles.cardTitle} numberOfLines={2}>{slot.title}</Text>
      <Text style={styles.cardLine}>
        🕒 {fmt24(slot.start)} ~ {fmt24(slot.end)} • {Math.max(10,slot.totalMins)}분 × 1 • <Text style={tint}>{toTimeString(slot.minsLeft)}</Text>
      </Text>
      <Text style={styles.cardLine}>👥 {slot.attendees.length}/{slot.max} • {stars(slot.proofScore)}</Text>
      ...
      <View style={styles.cardFoot}>
        <TouchableOpacity style={styles.outBtn} onPress={onDetails}><Text style={styles.outBtnT}>{T.details}</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.inBtn, isFull && {opacity:0.5}]} onPress={onPrimary} disabled={isFull || state!=="upcoming"}>
...
function Details({slot,onBack,onShare,onJoin,onLeave,nowMs}){
  ...
  const isFull = slot.attendees.length>=slot.max && !joined;
  const {state, secsToStart} = getState(nowMs, slot.start, slot.totalMins);
  ...
  const minText = slot.totalMins>=20 ? `최소 ${slot.totalMins}분` : `${slot.totalMins}분`;
  ...
  return(
    <View style={styles.detailWrap}>
      ...
      <ScrollView
        ...
        keyboardShouldPersistTaps="handled"
      >
        <TouchableOpacity onPress={onBack} hitSlop={12}><Text style={styles.back}>{T.back}</Text></TouchableOpacity>

        <View style={[styles.detailsBox,{borderColor:colorFor(slot.type)}]}>
          {/* 헤더 */}
          <View style={styles.badgeRow}>
            <MiniBadge text={`${iconFor(slot.type)} ${slot.type}`} tone={colorFor(slot.type)} />
            <MiniBadge text={labelText} tone={labelTone} />
          </View>
          <Text style={[styles.detailsTitle,{color:colorFor(slot.type)}]} numberOfLines={3}>{slot.title}</Text>
          <Text style={[styles.subBy]} numberOfLines={1}>💎 Hosted by <Text style={{fontWeight:"900"}}>{hostLabel}</Text></Text>
          ...
          <View style={styles.infoBlock}>
            ...
            <Text style={styles.infoLine}>👥 인원 {slot.max||2}명  🎯 분위기 {vibe}</Text>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.cardLine}>🕒 {fmt24(slot.start)} ~ {fmt24(slot.end)} • <Text style={tint}>{toTimeString(slot.minsLeft)}</Text></Text>
            <View style={styles.progOuter}><View style={[styles.progInner,{width:`${Math.max(4,ratio*100)}%`, backgroundColor:tint.color||"#6AAEFF"}]} /></View>
          </View>
          ...
          <View style={{flexDirection:"row",gap:10,marginTop:12}}>
            <TouchableOpacity style={styles.secondaryBtn} onPress={onShare}><Text style={styles.secondaryText}>{T.share}</Text></TouchableOpacity>
            {!joined && !isFull && state==="upcoming" && (
              <TouchableOpacity style={styles.primaryBtn} onPress={onJoin}><Text style={styles.primaryText}>{T.checkin}</Text></TouchableOpacity>
            )}
            {joined && state!=="ended" && (
...
function MiniBadge({text,tone}){ return(<View style={[styles.miniBadge,{backgroundColor:tone+"22",borderColor:tone}]}><Text style={[styles.miniBadgeT,{color:tone}]}>{text}</Text></View>); }
  ...
  return(
    <View style={styles.sheetWrap}>
      ...
      <View style={styles.sheetCard}>
        ...
        {options.map(opt=>(<TouchableOpacity key={opt} style={styles.sheetItem} onPress={()=>onPick(opt)}><Text style={[styles.sheetItemT, value===opt && {color:"#3EC6FF"}]}>{opt}{value===opt?" •":""}</Text></TouchableOpacity>))}
        <TouchableOpacity style={[styles.primaryBtn,{marginTop:6}]} onPress={onCancel}><Text style={styles.primaryText}>OK</Text></TouchableOpacity>
      </View>
    </View>
  );
}

/* ✅ Multi City Sheet (복수 선택) */
function MultiCitySheet({currentSet,onApply,onClose}){
  const [local,setLocal]=useState(new Set(Array.from(currentSet)));
  const toggle=(code)=> setLocal(prev=>{ const s=new Set(prev); s.has(code)?s.delete(code):s.add(code); return s; });
    ...
    <View style={styles.sheetWrap}>
      ...
      <View style={styles.sheetCardTall}>
        ...
        <ScrollView style={{maxHeight:420}} showsVerticalScrollIndicator={false} overScrollMode="never">
          {Object.keys(groups).map(gr=>(
            <View key={gr} style={{marginBottom:8}}>
              ...
              {groups[gr].map(c=>{
                const on = local.has(c.code);
                return (
                  <TouchableOpacity key={c.code} style={styles.cityRow} onPress={()=>toggle(c.code)}>
                    <View style={[styles.chk, on && styles.chkOn]}><Text style={[styles.chkT, on && styles.chkTOn]}>✓</Text></View>
        ...
        <View style={{flexDirection:"row",gap:8,marginTop:10,justifyContent:"flex-end"}}>
          <TouchableOpacity style={styles.secondaryBtn} onPress={reset}><Text style={styles.secondaryText}>{T.reset}</Text></TouchableOpacity>
          <TouchableOpacity style={styles.primaryBtn} onPress={apply}><Text style={styles.primaryText}>{T.apply}</Text></TouchableOpacity>
        </View>
      </View>
    </View>
  );
    ...
    <View style={styles.modalWrap}>
      ...
      <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==="ios"?"padding":undefined}>
        <View style={styles.modalHead}>
          ...
          <TouchableOpacity onPress={onClose} hitSlop={12}><Text style={styles.modalClose}>✕</Text></TouchableOpacity>
        </View>
        ...
        >
          ...
          <View style={styles.dualWrap}>
            {CATS.map(c=>(
              <TouchableOpacity key={c.key}
                ...
                style={[
                  styles.formChipHalf,{borderColor:c.color, justifyContent:"center"},
          ...
          {form._cityPick && (
            <CitySheetSingle current={form.city}
          ...
          <View style={styles.dualRow}>
            <TextInput style={[styles.input,styles.duo]} placeholder="HH:MM" placeholderTextColor="#738"
              value={form.start} onChangeText={t=>setForm(f=>({...f,start:t}))}/>
            <TextInput style={[styles.input,styles.duo]} placeholder="HH:MM" placeholderTextColor="#738"
              value={form.end} onChangeText={t=>setForm(f=>({...f,end:t}))}/>
          </View>
          ...
          <TextInput
            style={styles.input}
            placeholder={T.titlePH} placeholderTextColor="#738"
          ...
          <TextInput
            ...
            multiline placeholder={T.descPH} placeholderTextColor="#738"
  ...
  return(
    <View style={styles.sheetWrap}>
      ...
      <View style={styles.sheetCardTall}>
        ...
        <TouchableOpacity style={[styles.primaryBtn,{marginTop:10}]} onPress={onClose}><Text style={styles.primaryText}>{T.done}</Text></TouchableOpacity>
      </View>
    </View>
  );
}
...
function Picker({button,value,onPress}){
  return(
    <TouchableOpacity style={[styles.picker, styles.duo]} onPress={onPress}>
      <Text style={styles.pickerT} numberOfLines={1}>{button}: <Text style={{color:"#fff"}}>{value}</Text></Text>
    </TouchableOpacity>
  );
}

function defaultTitle(cat){
  if(cat==="Dating")  return "워크&토크 • 퍼스트룩";
  if(cat==="Friends") return "벤치 토크 • 캔딧";
  if(cat==="Workout") return "조깅&토크 10";
  return "토크 • 어니스트 10";
}
function defaultDesc(cat){
  if(cat==="Dating")  return "10분. 진짜. 심플.";
  if(cat==="Friends") return "두 사람, 한 주제.";
  if(cat==="Workout") return "가벼운 페이스; 이어폰 OFF.";
  return               "정직한 대화. 영업 금지.";
}
...
const styles = StyleSheet.create({
  ...
  toggleT:{color:"#9aa",fontWeight:"800"}, toggleTOn:{color:"#fff"},
